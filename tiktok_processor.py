...
class TikTokProcessor:
    ...
    async def _get_video_metadata_browser(self, url: str) -> Dict:
        ...
        try:
            ...
            await page.wait_for_selector('[data-e2e="video-desc"]', timeout=10000)
            
            # Extract metadata from the page
            metadata = {
                'author': await page.evaluate('() => document.querySelector("[data-e2e=\"user-username\"]")?.textContent'),
                'likes': await page.evaluate('() => document.querySelector("[data-e2e=\"like-count\"]")?.textContent'),
                'comments': await page.evaluate('() => document.querySelector("[data-e2e=\"comment-count\"]")?.textContent'),
                'views': await page.evaluate('() => document.querySelector("[data-e2e=\"view-count\"]")?.textContent'),
                'description': await page.evaluate('() => document.querySelector("[data-e2e=\"video-desc\"]")?.textContent'),
